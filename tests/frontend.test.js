...
test('mobile groups keep actions on the edge and refresh without a success toast', () => {
  ...
  expect(css).toContain('.group-edit { top: 4rem; right: 6.15rem; }');
  expect(css).toContain('.group-copy { top: 4rem; right: 3.4rem; }');
  expect(css).toContain('top: 4.5rem;');
  const serviceWorker = fs.readFileSync(path.join(staticRoot, 'service-worker.js'), 'utf8');
